...
// packages/shared/src(shared 源码根),供 content 扫描 globs 使用
const sharedSrcDir = fileURLToPath(new URL('..', import.meta.url)).replace(/\/$/, '')
...
export default defineConfig({
  ...
  content: {
    pipeline: {
      include: [
        // 默认 globs(匹配所有被 import 的模块，含 workspace link 进来的 shared 源码)
        /\.(vue|svelte|[jt]sx|mdx?|astro|elm|php|phtml|html)($|\?)/,
        // 也扫描 app 的 markdown 内容 (相对 app cwd)
        'docs/**/*.md',
        // shared 包源码的文件系统扫描 (绝对路径，不依赖 app cwd)
        `${sharedSrcDir}/**/*.{ts,vue}`,
      ],
    },
  },
