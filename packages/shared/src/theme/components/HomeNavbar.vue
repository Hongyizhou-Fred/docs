...
import { NAV_TABS_BY_REGION, type NavTab } from '../../config/tabs.config'

// 由 createDocsConfig 通过 vite define 注入:本 region 实际存在的顶级分类目录列表
...
function switchRegion(target: typeof REGIONS[number]) {
  ...
  if (!inBrowser) return
  // 跨 region = 跨 app(每个 region 是独立 VitePress 实例,由部署层按 /hk /sg
  // /us 前缀路由到对应产物),必须整页跳转;当前 app 的 SPA 路由表里没有目标页。
  // 保守起见直接落到目标 region 首页,不猜测对端是否存在同路径文章。
  window.location.assign(`/${target.code}/`)
  langOpen.value = false
}

// 当前 locale 直接取 VitePress 解析结果(root=en / zh-CN / zh-HK)
...
function switchLang(target: typeof ALL_LANGS[number]) {
  ...
  // 剥掉(可选的)region 与 locale 前缀,得到 region 内剩余路径。
  // route.path 不含 base,通常形如 /zh-CN/account/foo;region 前缀分支兜底
  // 防御历史调用形态。
  const p = route.path
  let rest = p
    .replace(/^\/(hk|sg|us)(?=\/|$)/, '')
  ...
  // 目标 locale 在该 region 下的前缀(默认语言 en 不带 locale 段)
...
const activeTab = computed(() => {
  // 剥掉(可选的)region 与 locale 前缀,再按当前 region 的 NAV_TABS 匹配相对
  // path。route.path 不含 base,region 分支只是防御。
  let p = route.path
    .replace(/^\/(hk|sg|us)(?=\/|$)/, '')
    .replace(/^\/(zh-CN|zh-HK)(?=\/|$)/, '') || '/'
  if (!p.startsWith('/')) p = '/' + p
